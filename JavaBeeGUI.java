...
public class JavaBeeGUI extends JComponent implements KeyListener, MouseListener, ActionListener {

   // ************* FUNCTIONS FOR STUDENTS TO CALL IN JavaBeeLogic.java
   // **************

   // Returns the text currently entered for the player guess as a STRING
   public static String getGuessStr() {
      return JavaBeeGUI.playerGuess;
   }

   // Returns the text currently entered for the player's guess as a CHAR ARRAY
   public static char[] getGuessArr() {
      char[] toReturn = new char[JavaBeeGUI.playerGuess.length()];
      for (int i = 0; i < JavaBeeGUI.playerGuess.length(); i++)
         toReturn[i] = JavaBeeGUI.playerGuess.charAt(i);

      return toReturn;
   }

   // Sets the player's guess text in the game window (above the hives) to the
   // argument String value.
   //
   // This function will throw an EXCEPTION if given too long a String
   public static void setGuess(String guess) {
      if (guess == null)
         throw new IllegalArgumentException("Error! null String passed to setGuess(...)!");
      else if (guess.length() > JavaBeeLogic.MAX_WORD_LENGTH)
         throw new IllegalArgumentException("Error! Too long String passed to setGuess(...)!: " + guess);
      JavaBeeGUI.playerGuess = guess;
   }

   // Returns the letter in the YELLOW (i.e. center) hive as a CHAR
   public static char getYellowHiveChar() {
      return hives.get(0).getLetter();
   }

   // Returns the letters in the gray (i.e. outer) hives as a CHAR ARRAY
   public static char[] getGrayHiveChars() {
   ...
   public static char[] getAllHiveChars() {
   ...
   // Adds the argument word to the word bank on the righthand side of the game
   // window. The second argument is the number of points the added word is worth;
   // this function then updates the score and RETURNS the new score total as an
   // int.
   //
   // This function will throw an EXCEPTION if given an invalid word.
   public static int addToGuessBank(String guess, int points) {

      if (guess == null)
         throw new IllegalArgumentException("Error! null String passed to addToGuessBank(...)!");
      else if (guess.length() < JavaBeeLogic.MIN_WORD_LENGTH)
         throw new IllegalArgumentException("Error! Word too short to be added to GuessBank!: " + guess);
      else if (guess.length() > JavaBeeLogic.MAX_WORD_LENGTH)
         throw new IllegalArgumentException("Error! Word too long to be added to GuessBank!: " + guess);

      guess = charToUpperCase(guess.charAt(0)) + guess.substring(1).toLowerCase();
      if (enteredWords.contains(guess))
         throw new IllegalArgumentException("Error! Word: \"" + guess + "\" already in GuessBank!");
      if (points < 0)
         throw new IllegalArgumentException("Error! points value <= 0 passed to addToGuessBank(...)!: " + points);

      enteredWords.add(guess);
      enteredWordsPoints.add(points);
      JavaBeeGUI.score += points;
      wordAdded = true;
      return JavaBeeGUI.score;
   }
   ...
   // Sets the Player's title, displayed to the left of their score.
   // A player's title changes as their score increases.
   //
   // This function will throw an EXCEPTION if given an invalid title
   public static void setTitle(String newTitle) {

      if (newTitle == null)
         throw new IllegalArgumentException("Error! null String passed to setTitle(...)!");
      else if (newTitle.length() == 0)
         throw new IllegalArgumentException("Error! empty String passed to setTitle(...)!" + title);
      else if (newTitle.length() > MAX_TITLE_LENGTH)
         throw new IllegalArgumentException("Error! too long String passed to setTitle(...): " + title);

      JavaBeeGUI.title = newTitle;
   }

   // Displays the argument error message in red text above the guess text
   // area. This function DOES NOT wiggle the player text.
   public static void displayError(String message) {

      if (message == null)
         throw new IllegalArgumentException("Error! null String passed to displayError(...)!");

      JavaBeeGUI.errorText = message;
      errorAlpha = 0;
   ...
   public static void wiggleGuess() {
   ...
   // Frequency (in milliseconds) that the window checks to see if repaint is
   // necessary,
   ...
   // Location attributes for center hive (other hive locations derived from
   // center)
   ...
   // Size and location attributes for Wordlist (which displays words player has
   // entered)
   ...
   // Location attributes for score and title text
   private static final int TITLE_X = WORDLIST_X - WORDLIST_BORDER_PADDING / 3;
   private static final int TITLE_Y = WORDLIST_Y - 40;
   private static final int SCORE_X = TITLE_X + 110;
   private static final int SCORE_Y = TITLE_Y - WORDLIST_BORDER_PADDING;
   // Font and color attributes for score/title text
   private static final int SCORE_CIRCLE_SIZE = 30; // background circle drawn behind score number
   private static final Font TITLE_FONT = new Font("SanSerif", Font.BOLD, 15);
   ...
   // Max length for a player's displayed title
   private static int MAX_TITLE_LENGTH = 10;

   // Location attributes for Debug text (only shown when one or more debug toggles
   // are enabled)
   ...
   // Governs speed/distance of player text "wiggle" animation (used to indicate
   // invalid input)
   ...
   // (kind of unintuitive, but necessary to keep everything non-OOP for the first
   // project!)
   private static JavaBeeGUI canvas;
   ...
   // The words in the wordlist and their respective point values; the two lists
   // work in tandem
   ...
   // Stores text entered by player and currently being displayed in the player
   // guess text field
   private static String playerGuess = "";
   // Indicates if a new word was just added to the wordlist (and triggers a
   // repaint)
   private static boolean wordAdded = false;

   // Player's current score and title
   private static int score = 0;
   private static String title = "Placeholder";

   // Tracks the number of timer ticks elapsed since the game launched (used for
   // animations)
   private static int ticksElapsed = 0;
   private static boolean cursorBlink = false;

   // Stores error text to be displayed and the state of its opacity (as it fades
   // in/out)
   ...
   public static void launchGame() {
      ...
      char[] letters = JavaBeeLogic.initGame();
      ...
      // Run the JavaBeeLogic warmup code before launching the game
      JavaBeeLogic.warmupStep2();
   }

   // Validates and normalizes characters to be displayed in the hives (provided as
   // an char array)
   private static void validateNormalizeLetters(char[] letters) {
      ...
      if (letters == null)
         throw new IllegalStateException(
               "ERROR! JavaBeeLogic's initGame() returned 'null' for its hive letters array!");
      else if (letters.length != JavaBeeLogic.HIVE_COUNT)
         throw new IllegalStateException(
               "ERROR! JavaBeeLogic's initGame() returned an array of the wrong size!:" + Arrays.toString(letters));
      ...
      for (int i = 0; i < letters.length; i++) {
         ...
         if (!ALL_LETTERS.contains(letters[i]))
            throw new IllegalStateException(
                  "ERROR! The array returned by JavaBeeLogic's initGame() contains a non-letter!:"
                        + Arrays.toString(letters));
         else if (!dupeCheck.add(letters[i]))
            throw new IllegalStateException(
                  "ERROR! The array returned by JavaBeeLogic's initGame() contains a duplicate!:"
                        + Arrays.toString(letters));
         if (i == 0)
      ...
      for (int i = 0; i < letters.length; i++) {
         if (JavaBeeLogic.VOWEL_CHARS.indexOf(letters[i]) >= 0)
            return;
      }
      throw new IllegalStateException(
            "ERROR! The array returned by JavaBeeLogic's initGame() contains no vowel!:" + Arrays.toString(letters));
   }
   ...
   private static void initWindow() {
      ...
      // Allows game to use mouse/keyPressed methods implemented in JavaBeeGUI.java
   ...
   private static void initButtons() {
      ...
      enterButton = new RoundRectangle2D.Double(ENTER_X, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, BUTTON_ARC_W,
            BUTTON_ARC_H);
      deleteButton = new RoundRectangle2D.Double(DELETE_X, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, BUTTON_ARC_W,
            BUTTON_ARC_H);
   }

   // Initialize GuessBank area (where previously entered words are displayed)
   private static void initGuessBank() {
   ...
   // Draws all the hives (both center and outer) on the game window, along with
   // their letters
   ...
   // Draws the GuessBank, which stores all the words the player has previously
   // entered (along with their point values)
   private void drawGuessBank(Graphics2D g2D) {
   ...
   // Draws the player's score and title to the game window
   public static void drawScoreInfo(Graphics2D g2D) {
      ...
      textDim = g2D.getFontMetrics().getStringBounds(title, g2D);
      fontX = (int) (SCORE_X - textDim.getWidth() - WORDLIST_BORDER_PADDING);
      g2D.drawString(title, fontX, TITLE_Y);
   }

   // Draws error message text, when one is being displayed
   public static void drawErrorMessage(Graphics g2D) {
      // use the error alpha value (ie opacity) to deterimine if an error is currently
      // being displayed
      if (errorAlpha <= 0)
         return;
   ...
   // Handles updating opacity of fading text when an error message is being
   // displayed
   ...
   private static void handleWiggle() {
      ...
      // If the wiggle has gone too far right/left, starting wiggling the opposite
      // direction
   ...
   public void keyPressed(KeyEvent event) {
      ...
      // Forward the pressed key on to JavaBeeLogic, so long as it is an alpha or
      // Backspace/Enter key
      if (ALL_LETTERS.contains(keyChar) || keyChar == JavaBeeLogic.BACKSPACE_KEY || keyChar == JavaBeeLogic.ENTER_KEY) {
         JavaBeeLogic.reactToKey(keyChar);
         repaint();
      }
   }
   ...
   public void mousePressed(MouseEvent event) {
      ...
      if (enterButton.contains(clickX, clickY)) {
         JavaBeeLogic.reactToKey(JavaBeeLogic.ENTER_KEY);
      } else if (deleteButton.contains(clickX, clickY)) {
         JavaBeeLogic.reactToKey(JavaBeeLogic.BACKSPACE_KEY);
      }
      // if a hive was clicked...
      else {
         for (Hive h : hives) {
            if (h.wasClicked(clickX, clickY)) {
               JavaBeeLogic.reactToKey(h.getLetter());
               break;
            }
         }
      }
